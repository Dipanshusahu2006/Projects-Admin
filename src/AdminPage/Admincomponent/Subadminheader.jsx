...
function SubAdminhaeder() {
  ...
  // ✅ Wrapped async fetch in useCallback for stable reference
  const SubadminProfiles = useCallback(async () => {
    try {
      const response = await fetch(`https://main-projectnode.vercel.app/admin/Get/${Id2}`);
      const data = await response.json(); // ✅ fixed whitespace error here
